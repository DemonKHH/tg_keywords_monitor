...
# Telethon 客户端初始化并启动
def start_telethon(application):
    async def run_client():
        client = TelegramClient(USER_SESSION, int(API_ID), API_HASH)
        await client.start()
        client.add_event_handler(lambda event: handle_new_message(event), events.NewMessage)
        logger.info("Telethon 客户端已启动，开始监听消息...")
        await client.run_until_disconnected()
...
def main():
    ...
    telethon_thread = threading.Thread(target=start_telethon, args=(application,), daemon=True)

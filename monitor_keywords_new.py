...
class UserConfigManager:
    ...
    async def get_interval(self, user_id):
        async with self.lock:
            return self.db_manager.get_user_interval(user_id)

    async def set_interval(self, user_id, interval):
        async with self.lock:
            self.db_manager.set_user_interval(user_id, interval)
...
class TelegramBot:
    def __init__(self, token, admin_ids, admin_username, api_id, api_hash,  db_path='bot.db'):
        ...
        self.user_config_manager = UserConfigManager(self.db_manager, self.config_manager)
    ...
    def setup_handlers(self):
        ...
        self.application.add_handler(CommandHandler("add_group", self.add_group))
        self.application.add_handler(CommandHandler("remove_group", self.remove_group))
    ...
    async def send_escaped_message(self, chat_id, text, parse_mode='MarkdownV2', **kwargs):
        escaped_text = escape_markdown(text, version=2) if parse_mode == 'MarkdownV2' else text
    ...
    async def handle_new_message(self, event, uid):
        ...
        try:
            ...
            # user_monitored_groups = self.db_manager.get_user_monitored_groups(uid)
            # # 仅处理来自被监听的群组的消息
            # if chat_id not in [group_id for groups in user_monitored_groups for group_id in groups]:
            #     logger.debug(f"消息来自非被监听群组 ({chat_id})，忽略。")
            #     return
    ...
    async def add_group(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if len(context.args) < 1:
            await update.message.reply_text(
                "❌ 请提供群组ID。例如：`/add_group -1001234567890`",
                parse_mode='Markdown'
            )
            logger.debug("add_group 命令缺少参数。")
            return

        try:
            raw_group_id = int(context.args[0])
            # group_id = self.validate_group_id(raw_group_id)
            group_id = raw_group_id
            logger.debug(f"用户 {user_id} 添加群组 {group_id}")
        except ValueError as ve:
            await update.message.reply_text(
                "❌ 群组ID必须是整数。例如：`/add_group -1001234567890`",
                parse_mode='Markdown'
            )
            logger.debug(f"add_group 命令参数错误: {ve}")
            return

        # 获取用户所有已认证的 Telethon 客户端
        accounts = self.db_manager.get_user_accounts(user_id)
        authenticated_accounts = [acc for acc in accounts if acc[4] == 1]  # is_authenticated

        if not authenticated_accounts:
            await update.message.reply_text(
                "⚠️ 您没有已认证的 Telegram 账号。请使用 `/login` 命令进行登录。",
                parse_mode='Markdown'
            )
            logger.error(f"用户 {user_id} 没有已认证的 Telegram 账号。")
            return
        ...
        for account in authenticated_accounts:
            ...
            if not client:
                # 初始化 Telethon 客户端
                client = TelegramClient(f'sessions/user_{user_id}_{account_id}', account[3], account[4])
                if account[5]:  # session_string
                    client.session.load_string(account[5])
                await client.connect()
                self.user_clients[account_id] = client
            ...
            try:
                ...
                if participant.participant:
                    joined_accounts.append(account_id)
                    if group_name == "未知群组":
                        group_entity = await client.get_entity(group_id)
                        group_name = group_entity.title if hasattr(group_entity, 'title') else "未知群组"
            except errors.UserNotParticipantError:
                continue
            except Exception as e:
                await update.message.reply_text(
                    f"❌ 检查账号 {account_id} 是否加入群组时出错：{e}",
                    parse_mode='Markdown'
                )
                logger.error(f"检查账号 {account_id} 是否加入群组 {group_id} 时出错：{e}", exc_info=True)

        if not joined_accounts:
            await update.message.reply_text(
                "❌ 您的所有已认证账号均未加入该群组。请先通过其中一个账号加入群组。",
                parse_mode='Markdown'
            )
            logger.error(f"用户 {user_id} 的所有已认证账号均未加入群组 {group_id}。")
            return

        # 将群组添加到用户监听列表
        try:
            self.db_manager.add_group(user_id, group_id, group_name)
            await update.message.reply_text(
                f"✅ 群组 `{group_id}` - *{group_name}* 已添加到您的监听列表。",
                parse_mode='Markdown'
            )
            logger.info(f"用户 {user_id} 添加了群组 {group_id} - {group_name} 到自己的监听列表。")

            # 监听该群组的消息
            for account_id in joined_accounts:
                client = self.user_clients.get(account_id)
                if client:
                    client.add_event_handler(
                        lambda event, uid=user_id: self.handle_new_message(event, uid),
                        events.NewMessage(chats=group_id)
                    )
                    logger.info(f"账号 {account_id} 开始监听群组 {group_id}。")
        except Exception as e:
            await update.message.reply_text(
                f"❌ 无法将群组添加到监听列表。\n错误详情: {e}",
                parse_mode='Markdown'
            )
            logger.error(f"将群组 {group_id} 添加到监听列表失败: {e}", exc_info=True)
    ...
    async def remove_group(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        groups = self.db_manager.get_user_monitored_groups(user_id)
        ...
        if not groups:
            ...
            logger.info(f"用户 {user_id} 请求移除群组，但没有被监听的群组。")
            return
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("""
                ...
                FROM groups 
                WHERE group_id IN (
                    SELECT group_id FROM user_monitored_groups WHERE user_id = ?
                )
            """, (user_id,))
        ...
        # 创建按钮列表，每个按钮代表一个群组ID和名称
        keyboard = []
        for group_id, group_name in groups:
            button_text = f"{group_id} - {group_name}"
            keyboard.append([InlineKeyboardButton(button_text, callback_data=f"remove_group:{group_id}:{user_id}")])
        ...
        logger.info(f"向用户 {user_id} 显示移除群组按钮。")
    ...
    async def list_groups(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not groups:
            await update.message.reply_text("ℹ️ 您当前没有监听任何群组。", parse_mode='Markdown')
            logger.info(f"用户 {user_id} 请求列出群组，但没有被监听的群组。")
            return

        # 格式化群组列表
        group_list = '\n'.join([f"• `{gid}` - *{gname}*" for gid, gname in groups])

        await update.message.reply_text(f"📋 *您当前监听的群组列表：*\n{group_list}", parse_mode='Markdown')
        logger.info(f"用户 {user_id} 列出了自己的监听群组。")
